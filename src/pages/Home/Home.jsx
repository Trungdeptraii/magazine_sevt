...
import { ContaintStatus,HomeContainer, HomePreview, HomeStatus, ItemStatus, Square, SquareTitle, TitleItemStatus, fnTaskStatus, getField, handleCheckConfig, handleDisabledButton, handleFetchIdModel, sendRun } from './homee'
...
const Home = () => {
  ...
  const {amr: {data, sensor}} = useSelector((state)=>{return state.amr})
  ...
  const [checkMagazine, setCheckManazine] = useState("Không có Magazine")
  ...
  useEffect(()=>{
    if(dataMagazine.magazine.line45 || dataMagazine.magazine.line46){
      if(checkMagazine == "Không có Magazine"){
        setCheckManazine("Có Magazine")
      }
    }else if(!dataMagazine.magazine.line45 && !dataMagazine.magazine.line46){
      if(checkMagazine != "Không có Magazine"){
        setCheckManazine("Không có Magazine")
      }
    }
  },[dataMagazine.magazine.line45, dataMagazine.magazine.line46])
    ...
    <HomeContainer>
      ...
      <HomePreview>
        <div style={{height: "calc(100vh - 330px)"}}>
          ...
          <HomeMagazine >
            <Badge onHandle={handleDataMagazine} controlAMR={controlAMR}/>
            <InfoHomeMagazine>
              <PointMoveHomeMagazine>
                <TitleInfoMagazine>Chu trình chạy của AMR</TitleInfoMagazine>
              </PointMoveHomeMagazine>
              <ShowJigHomeMagazine>
                <InfoJig>
                  <TitleInfoMagazine>Kiểm tra Jig</TitleInfoMagazine>
                  <ContentInfoJig>
                    <ContentInfoJigItem style={sensor && sensor[0] == 1 ? {...senserON.parent} : {...sensorOFF.parent}}>
                      <InfoJigItemIcon>
                        <FaParachuteBox style={sensor && sensor[0] == 1 ? {...senserON.children} : {...sensorOFF.children}}/>
                      </InfoJigItemIcon>
                      <InfoJigItemTitle>VT 1</InfoJigItemTitle>
                    </ContentInfoJigItem>
                    <ContentInfoJigItem style={sensor && sensor[1] == 1 ? {...senserON.parent} : {...sensorOFF.parent}}>
                      <InfoJigItemIcon>
                        <FaParachuteBox style={sensor && sensor[1] == 1 ? {...senserON.children} : {...sensorOFF.children}}/>
                      </InfoJigItemIcon>
                ...
                <InfoMagazine>
                  <TitleInfoMagazine>Kiểm tra Magazine</TitleInfoMagazine>
                  <ContentInfoMagazine>
                    <span style={{textAlign: "center", lineHeight: 1.2}}>{checkMagazine}</span>
                  </ContentInfoMagazine>
                </InfoMagazine>
              </ShowJigHomeMagazine>
            </InfoHomeMagazine>
        ...
        {
          RobotName == "Rhythm" ? "" : <Space style={{marginTop: 10, width: "100%", display: "flex", justifyContent: "space-evenly", gap: 20}}>
          <Buttonn onClick={handleRunAMR} style={{backgroundColor: controlAMR.run ? "limegreen" : "#1677ff"}}>Chạy</Buttonn>
          <Buttonn onClick={handleRunStop} style={{backgroundColor: taskStatus == 3 ? "orange": "#1677ff"}}>{taskStatus == 3 ? "Tiếp tục" : "Dừng"}</Buttonn>
          <Buttonn onClick={handleCancelAMR}>Hủy</Buttonn>
          <Buttonn onClick={handleGoStandBy}>StandBy</Buttonn>
          <Buttonn onClick={handleGoCharge}>Sạc</Buttonn>
          <Buttonn onClick={handleClickButton}>Button</Buttonn>
        </Space>
        }
      </HomePreview>
    </HomeContainer>
