...
const Drawerr = ({open, setOpen, arrPoint, onDelete}) => {
    const onClose = ()=>{
      setOpen(false)
    }
  return (
            ...
            <InfiniteScroll dataLength={arrPoint} scrollableTarget="scrollableDiv" style={{overflow: 'unset'}}>    
                <List
                    ...
                    <List.Item style={{display: 'flex', gap: '15px'}}>
                      ...
                      {
                        index != 0 ? <Button onClick={()=>{onDelete(item)}} style={{height: '72px', display: 'block', fontWeight: 600, backgroundColor: 'orange'}}>Xóa</Button> : ""
                      }
